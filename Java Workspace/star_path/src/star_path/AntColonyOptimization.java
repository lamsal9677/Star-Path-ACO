...
public class AntColonyOptimization {
    ...
    //set all ants to start from position 0
    ...
    private void updateTrails() {
    	//half of the trails get evaporated to provide better result
    	for (int i = 0; i < numberOfCities; i++) {
            for (int j = 0; j < numberOfCities; j++) {
                trails[i][j] *= evaporation;
            }
        }
        //for the good path add more phermones
        for (Ant a : ants) {
    ...
    private void clearTrails() {
        IntStream.range(0, numberOfCities)
            .forEach(i -> { IntStream.range(0, numberOfCities) .forEach(j -> trails[i][j] = c);});  
    }
}
